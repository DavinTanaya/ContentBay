...
import { Table, Tag, Button, Select, Avatar, Dropdown, Menu, Modal, Empty, message, Popconfirm } from 'antd';
import { PlusOutlined, MoreOutlined, UserOutlined, ClockCircleOutlined, DeleteOutlined } from '@ant-design/icons';
...
export const ContentListView: React.FC = () => {
  ...
  const workspaceModels = React.useMemo(() => {
    return allModels.filter((m) => (m.workspaceId || 'project-1') === activeWorkspaceId);
  }, [allModels, activeWorkspaceId]);

  // 2. Fetch Content Entries
  const activeModelFilter = selectedModelId === 'All' ? undefined : selectedModelId;
  const { data: contentsData, loading: contentsLoading, refetch } = useGetContentsApi(activeWorkspaceId, activeModelFilter);
  const contents = contentsData?.getContents || [];

  // 3. Delete Mutation
  const [deleteContent] = useDeleteContentApi(activeWorkspaceId, activeModelFilter);
  ...
  const confirmDelete = (id: string) => {
    Modal.confirm({
      ...
      content: 'Are you sure you want to delete this content entry? This action cannot be undone.',
      okText: 'Yes, Delete',
      okType: 'danger',
      cancelText: 'No',
      centered: true,
  ...
  const columns = [
    {
      ...
      render: (record: any) => {
        ...
        const title = payload.title || payload.name || payload.label || payload.heading || Object.values(payload)[0] || 'Untitled';
        return (
          <div className="flex flex-col">
            <span className="font-bold text-gray-900 line-clamp-1">{String(title)}</span>
            <span className="text-[10px] font-mono text-gray-400">ID: {record.id}</span>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      key: 'updatedAt',
      render: (record: any) => {
        const dateStr = record.updatedAt ? new Date(parseInt(record.updatedAt) || record.updatedAt).toLocaleDateString('en-US', {
          day: '2-digit',
          month: 'short',
          year: 'numeric'
        }) : '-';
    ...
    },
    {
      ...
      render: (record: any) => {
        ...
        return (
          <Tag color={bgColor} className="border-none font-bold text-[10px] rounded-lg px-3 py-0.5 m-0 capitalize" style={{ color }}>
            {status}
          </Tag>
        );
      },
        ...
        const rowMenuItems = [
          {
            key: 'edit',
            label: 'Edit Entry',
            onClick: () => navigate(`/workspace/${activeWorkspaceId}/content/create?modelId=${record.contentModelId}&entryId=${record.id}`)
          },
          {
          ...
          {
            key: 'delete',
            label: <span className="text-red-500 flex items-center gap-2"><DeleteOutlined /> Delete</span>,
  ...
  const filteredContents = React.useMemo(() => {
    return contents.filter(item => {
      ...
      return (item.status || 'draft').toLowerCase() === statusFilter.toLowerCase();
    });
  }, [contents, statusFilter]);

  const handleAddEntryClick = () => {
    if (workspaceModels.length === 0) {
      message.warning('Please create at least one Content Model first before adding content entries.');
      return;
    }
  ...
  const selectModelMenuItems = workspaceModels.map(model => ({
    ...
    onClick: () => {
      ...
      navigate(`/workspace/${activeWorkspaceId}/content/create?modelId=${model.id}`);
    }
  }));
      ...
      <main className="flex-grow p-12 bg-white">
        <div className="flex justify-between items-start mb-10">
          <div>
            ...
            <p className="text-gray-500 font-medium">Manage and publish your dynamic content entries</p>
          </div>
        ...
        <div className="flex items-center gap-8 mb-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <Select 
              ...
              variant="borderless"
            >
              {['All', 'Published', 'Draft'].map(s => <Select.Option key={s} value={s}>{s}</Select.Option>)}
            </Select>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm overflow-hidden mb-8">
          <Table 
            ...
            locale={{
              emptyText: <Empty description="No content entries found. Let's create your first entry!" />
            }}
